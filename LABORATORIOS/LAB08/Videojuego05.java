...
public class Videojuego05 {
    ...
    public static HashMap<Integer,Soldado> CrearEjercito(){
        ...
        for(int i =0;i<n;i++){
                ...
                aux=new Soldado(Nombre,Fila,Columna,Vida);
                Ejercito.put(i, aux);
    ...
    public static Soldado mayorVida(HashMap<Integer,Soldado> a){
        ...
        for(int i=0;i<a.size();i++){
            if(a.get(i).getVida()>mayor){
                Mayor=a.get(i);
                mayor=a.get(i).getVida();
            }
    ...
    public static HashMap<Integer,Soldado> OrdenarPorVida(HashMap<Integer,Soldado> a){
        ...
        for(int x=0;x<a.size()-1;x++){
            int mayor=a.get(x).getVida(),pos=x;
            for(int i=x+1;i<a.size();i++){
               if(mayor<a.get(i).getVida()) {
                    mayor=a.get(i).getVida();
                    pos=i;
                }  
            }
    ...
    public static void imprimirVida(HashMap<Integer,Soldado> a){
        for(int i=0;i<a.size();i++){
            System.out.println(a.get(i).getNombre()+" \tVida: "+a.get(i).getVida());
        }
    }
    public static HashMap<Integer,Soldado> OrdenarVidaInsercion(HashMap<Integer,Soldado> a){
        ...
        for(int x=1;x<a.size();x++){
            aux=a.get(x);
            auxiliar=a.get(x).getVida();
            y=x-1;
            while(y>-1 && a.get(y).getVida()>auxiliar){
                a.put(y+1, a.get(y));
                y=y-1;
            }
            a.put(y+1, aux);
